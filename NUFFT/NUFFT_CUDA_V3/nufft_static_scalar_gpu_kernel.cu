 ...
 */
/*
* nufft_static_scalar_gpu_kernel.cu: kernels used by NufftStaticScalarGpu
*/
...
__global__ void nufft_project_static_scalar_cubic(FP_TYPE *_d_test, NufftArrayGpuStaticScalar *nufft_array, NufftParamGpu *nufft_const, GpuExecParam *param)
{
	...
	__shared__ FP_TYPE s_src_amp[BLOCK_SIZE_PROJ_INTERP]; //shared memory array to store the source amplitudes
	...
	FP_TYPE _P[1]; // used to store final fields. declared as an array as it might have more than 1 components for vector fields
	_P[0] = 0.0f;
	...
	while (_shared_start < _num_src2)
	{
		...
		if (_shared_start + _obs_idx < _num_src2)
		{
			...
			s_src_amp[_tidx] = nufft_array->d_src_amp[_global_src_idx_start2 + _shared_start + _obs_idx];
		}
		...
		for (int i = 0; (i < interp_nodes_per_box) && (i + _shared_start < _num_src2); i++)
		{
			...
			_P[0] += _interp_coeff * s_src_amp[i + _shared_offset];
		} // i
		__syncthreads();
		_shared_start += interp_nodes_per_box;	
	} // _shared_start

	nufft_array->d_u_src_grid_dev[_local_box_idx * interp_nodes_per_box + _obs_idx] = _P[0];
...
__global__ void nufft_project_static_scalar_linear(FP_TYPE *_d_test, NufftArrayGpuStaticScalar *nufft_array, NufftParamGpu *nufft_const, GpuExecParam *param)
{
	...
	__shared__ FP_TYPE s_src_amp[BLOCK_SIZE_PROJ_INTERP];
...
__global__ void nufft_fft_prep_static_scalar_cubic(FP_TYPE *_d_test, NufftArrayGpuStaticScalar *nufft_array, NufftParamGpu *nufft_const, GpuExecParam *param)
{
	...
	for (int i = 0; i < _box_idx[8]; i++)
	{
		...
		_P[0] += nufft_array->d_u_src_grid[_u_src_grid_addr];
#ifdef _GPU_D_TEST
	_d_test[_gid] = size_t(_u_src_grid_addr);
#endif
	}

	nufft_array->d_fft_inplace_r2c_FP[_cufft_in_addr] = _P[0];
...
__global__ void nufft_convolution_static_scalar(FP_TYPE *_d_test, NufftArrayGpuStaticScalar *nufft_array, NufftParamGpu *nufft_const, GpuExecParam *param)
{
	...
	__shared__ CUFFT_COMPLEX_TYPE s_u_src_grid_k[BLOCK_SIZE_CONV];
	__shared__ FP_TYPE s_g_grid_k[BLOCK_SIZE_CONV];//IMP MAT CHANGE!!!SHOULD BE LESS!!!!IF THIS SUBROUTINE IS LIMITED BY SHARED MEMORY, THEN
	...
	if( j >= nufft_const->green_size[1] )
		j = nufft_const->fft_size[1] - j;
	if( k >= nufft_const->green_size[2])
		k = nufft_const->fft_size[2] - k;
	int _green_id = i + j*nufft_const->green_size[0] + k*nufft_const->green_size[0]*nufft_const->green_size[1];

	s_u_src_grid_k[_tid] = nufft_array->d_fft_inplace_r2c[_gid];
	s_g_grid_k[_tid] = nufft_array->d_k_imp_mat_data_gpu[_green_id];

	nufft_array->d_fft_inplace_r2c[_gid].x = s_u_src_grid_k[_tid].x * s_g_grid_k[_tid];
	nufft_array->d_fft_inplace_r2c[_gid].y = s_u_src_grid_k[_tid].y * s_g_grid_k[_tid];
	nufft_array->d_fft_inplace_r2c[_gid].x /= FP_TYPE(nufft_const->total_num_fft_pts);
	nufft_array->d_fft_inplace_r2c[_gid].y /= FP_TYPE(nufft_const->total_num_fft_pts);
#ifdef BEN_DEBUG_FFT
...
__global__ void nufft_fft_postp_static_scalar_cubic(FP_TYPE *_d_test, NufftArrayGpuStaticScalar *nufft_array, NufftParamGpu *nufft_const, GpuExecParam *param)
{
	...
	nufft_array->d_u_obs_grid[_gid] = nufft_array->d_fft_inplace_r2c_FP[obs_idx_glb];
...
{ 
	// nufft_correct_static_scalar_cubic calculate the inaccurate field generated by near field sources again and subtract them from the total field
...
//	unsigned int _obs_idx_glb;
	...
	__shared__ FP_TYPE s_src_amp[BLOCK_SIZE_CORRECT];
      
	FP_TYPE _Q1[1]; // An array stores far field on observer
	_Q1[0] = 0.0f;
	...
	//			direct_grid_interact_cubic_static_scalar(_r1, _Q1, s_src_amp + _shared_offset, cell_size, epsilon);
	//			//__syncthreads();
	//		}
	//	}
	//}// _near_box_counter
...
//	s_obs_amp[_tidx] = d_u_obs_grid[_obs_idx_glb]; 
	nufft_array->d_u_obs_grid_dev[_local_box_idx * interp_nodes_per_box + _obs_idx] -= _Q1[0];
...
{ 
	...
	for( int _near_box_counter = 0; _near_box_counter < total_num_near_box_per_box; _near_box_counter++){
		...
		direct_grid_interact_linear_static_scalar(_r1, _Q1, s_src_amp + _shared_offset, cell_size, epsilon);
...
{ 
	...
	while( near_box_left > 0 ){
		...
		for( int _near_box_counter = 0; _near_box_counter < near_box_left && _near_box_counter < BLOCK_SIZE_CORRECT; _near_box_counter++){
			...
			// linear: 0% of computational time of this subroutine goes here
			//if( _tidx < interp_nodes_per_box)
				direct_grid_interact_linear_static_scalar(_r1, _Q1, s_src_amp, cell_size, epsilon);
				__syncthreads();
		} // _near_box_counter
		near_box_left -= BLOCK_SIZE_CORRECT;
		s_near_list_start += BLOCK_SIZE_CORRECT; 
...
//	unsigned int _obs_idx_glb;
	...
	__shared__ FP_TYPE s_obs_amp[BLOCK_SIZE_PROJ_INTERP];
...
//	 Field amplitudes on the observer grid, with inaccurate near field subtracted
	s_obs_amp[_tidx] = nufft_array->d_u_obs_grid_dev[_local_box_idx * interp_nodes_per_box + _obs_idx]; 
	__syncthreads();
	...
	//	// Calculating far field, interpolation from the grid points to the observers
		...
		lagrange_interp_cubic(_Q1, _r_norm, s_obs_amp + _shared_offset);

		if (_shared_start2 + _obs_idx < _num_src2)
		{
				nufft_array->d_field_amp[_glb_src_idx_start2 + _shared_start2 + _obs_idx] = _Q1[0];
		}
...
//	int const src_size_dev = nufft_const->src_size_dev;
	...
	while (_shared_start2 < _num_src2)
	{
		...
		lagrange_interp_linear(_Q1, _r_norm, s_obs_amp + _shared_offset);
...
__global__ __launch_bounds__(BLOCK_SIZE_NEAR, 16) 
void nufft_exact_near_field(FP_TYPE *_d_test, NufftArrayGpuStaticScalar *nufft_array, NufftParamGpu *nufft_const, GpuExecParam *param)
{  
	unsigned int _tidx = threadIdx.x;
	unsigned int _bdim = blockDim.x;
	unsigned int _bid = blockIdx.x + blockIdx.y * gridDim.x;
	unsigned int _gid = threadIdx.x + (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x;
 	unsigned int _local_box_idx;
	unsigned int _box_idx;
	int const _block_size = BLOCK_SIZE_NEAR; //Sidi added unsigned, but error results
	unsigned int const src_size_dev = nufft_const->src_size_dev; //Sidi added unsigned 04/04/2013
	unsigned int const obs_size_dev = src_size_dev; //Sidi added unsigned 04/04/2013
	//	int const obs_size_dev = nufft_const->obs_size_dev;
	FP_TYPE const epsilon = nufft_const->epsilon;
	...
	__shared__ FP_TYPE s_src_amp[BLOCK_SIZE_NEAR];
		...
		while( s_near_list_start < _total_num_near_box ){
			...
			while (_s_addr_end >= 0)//fetch all near srcs
			{
				...
				if (_tidx < _s_addr_end) 
				{
					...
					s_src_amp[_tidx] = nufft_array->d_src_amp[_g_addr_start + _tidx];
				}
				...
				while (_fetch)// until shared mem is full or run out of near box
				{
					...
					_near_src = tex1Dfetch(tex_box_int, _near_box_idx + nufft_const->total_num_boxes * 2);
					_g_addr_start = tex1Dfetch(tex_box_int, _near_box_idx);
#else
					_near_src = nufft_array->d_obs_box_map[_near_box_idx + nufft_const->total_num_boxes * 2];
					...
					if (_tidx >= _s_addr_start && _tidx < _s_addr_end) 
					{
						...
						s_src_amp[_tidx] = nufft_array->d_src_amp[_g_addr_start - _s_addr_start + _tidx];
					}
				} // fetch
				__syncthreads();
				...
				for (int i = 0; i < _block_size && i < _s_addr_end; i++)
				{
					...
					get_field_static_scalar(_r1, &_magn, &_Q2, epsilon);
				} // i 
...
//#endif
		...
		if (_shared_start2 + _tidx < _num_src2)
		{
				nufft_array->d_field_amp[_glb_src_idx_start2 + _shared_start2 + _tidx] += _Q2;
