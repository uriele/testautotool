 ...
 */
/*
* nufft_static_scalar.cpp: class definition of Class NufftStaticScalar
*/
#include "error.h"
...
NufftStaticScalar :: NufftStaticScalar(class NBODYFAST *n_ptr) : Nufft(n_ptr)
{
	k_u_src_grid = NULL;
	u_src_grid = NULL;
	k_u_obs_grid = NULL;
...
NufftStaticScalar :: ~NufftStaticScalar()
{

	nbodyfast->memory->free_host<FP_TYPE>(&g_direct_near);
	nbodyfast->memory->free_host<ImpMatrixStaticScalar>(&g_grid_near);
	nbodyfast->memory->free_host<ImpMatrixStaticScalar>(&g_grid);
...
int NufftStaticScalar :: preprocessing()
{

	std::cout << "NufftStaticScalar :: preprocessing()" << std::endl;
	
	Nufft::preprocessing(); // above everything, call preprocessing() of its parent class
	
	// pointers to the impedance matrix
	g_grid = dynamic_cast<ImpMatrixStaticScalar*>(imp_mat_ptr);
	g_grid_near = dynamic_cast<ImpMatrixStaticScalar*>(near_imp_mat_ptr);

	// if we are using CPU, then build the following arrays, including a smaller FFT grid for near-field correction
	if (gpu_on == false)
	{
		nbodyfast->memory->alloc_host<FP_TYPE>(&u_src_grid, total_num_grid_pts, "nufft->u_src_grid");
		...
		nbodyfast->memory->alloc_host<FP_TYPE>(&u_near_src_grid, total_num_near_grid_pts, "nufft->u_near_src_grid");
		nbodyfast->memory->alloc_host<FP_TYPE>(&u_near_obs_grid, interp_nodes_per_box, "nufft->u_near_obs_grid");
		nbodyfast->memory->alloc_host<std::complex<FP_TYPE> >(&k_u_near_src_grid, total_num_near_grid_pts * 8, "nufft->k_u_near_src_grid");//!!!!!!!!!!!!!NEAR CHANGE!!!!!!!!!!!!!

		if (cpu_near_onfly == false)//!!!!!!!!!!!!!NEAR CHANGE!!!!!!!!!!!!!
		{
			...
			nbodyfast->memory->alloc_host<FP_TYPE>(&g_direct_near, problem_size * max_near_src_cnt, "nufft->g_direct_near");

			for (int i = 0; i < problem_size * max_near_src_cnt; i++)
			{
				...
				for (int m = obs_box_map[i]; m < obs_box_map[total_num_boxes + i]; m++)
				{
					...
					for (int j = 0; j < near_box_list[i * (total_num_near_box_per_box+1)]; j++)
					{
						...
						for (int l = src_box_map[_near_box_idx]; l < src_box_map[total_num_boxes + _near_box_idx]; l++, _cnt_src++)
						{
							...
							for (int k = 0; k < 3; k++) 
							{
								r[k] = (field_static_scalar->src_coord[l + k * problem_size] - field_static_scalar->obs_coord[m + k * problem_size]);
							}
							...
							FieldStaticScalar::get_G(_G_temp, _r_temp, Field::epsilon());

							g_direct_near[_idx_near_temp] = _G_temp[0]; 
						} // l
					} // j
				} // m
			} // i
...
int NufftStaticScalar :: g_mat_alloc(class ImpMatrix **_g_mat, int n_grid_ptr[3])
{
	class ImpMatrixStaticScalar *_imp_matrix = NULL;
	nbodyfast->memory->alloc_host<ImpMatrixStaticScalar>(&_imp_matrix, 1, "nufft->_g_mat");
	*_g_mat = _imp_matrix;
	return 0;
}

int NufftStaticScalar :: g_mat_set(class ImpMatrix *_g_mat, FP_TYPE *_src_grid_coord, FP_TYPE *_obs_grid_coord, int n_grid_ptr[3], int fft_size[3])
{
	// I believe this subroutine are not yet working when _src_grid_coord and _obs_grid_coord are not overlapped. So I put this line here
	 if (field_static_scalar->src_obs_overlap == false)
	 {
			std::cout << "The code is not yet working for src_obs_overalp = false at int NufftStaticScalar :: g_mat_set" << std::endl;
		 exit(0);
	 }

	 class ImpMatrixStaticScalar *_imp_matrix = dynamic_cast<ImpMatrixStaticScalar*>(_g_mat);
	...
	int _num_padded_grid_pts = fft_size[0]*fft_size[1]*fft_size[2]; //total_num_fft_pts; // number of points in the padded NUFFT grid  
	...
	nbodyfast->memory->alloc_host<FP_TYPE>(&_imp_matrix->imp_mat_data, _num_padded_grid_pts, "field->_imp_matrix->imp_mat_data");
	nbodyfast->memory->alloc_host<std::complex<FP_TYPE> >(&_imp_matrix->k_imp_mat_data, _num_padded_grid_pts, "field->_imp_matrix->k_imp_mat_data");
	nbodyfast->memory->alloc_host<FP_TYPE>(&_imp_matrix->k_imp_mat_data_gpu, _total_num_green_pts, "field->_imp_matrix->k_imp_mat_data_gpu");
...
//				int _idx_g_grid = i + j * fft_size[0] + k * fft_size[0] * fft_size[1];
				...
				FieldStaticScalar::get_G(_g_temp, _r_temp, Field::epsilon());
				_imp_matrix->imp_mat_data[i + j * fft_size[0] + k * fft_size[0] * fft_size[1]] = _g_temp[0];

				for (int l = 0; l < 3; l++) _r_temp[l] = r[l];
				_r_temp[0] = -_r_temp[0];
				FieldStaticScalar::get_G(_g_temp, _r_temp, Field::epsilon());
				if (i > 0)
					_imp_matrix->imp_mat_data[(fft_size[0] - i) + j * fft_size[0] + k * fft_size[0] * fft_size[1]] = _g_temp[0];

				for (int l = 0; l < 3; l++) _r_temp[l] = r[l];
				_r_temp[1] = -_r_temp[1];
				FieldStaticScalar::get_G(_g_temp, _r_temp, Field::epsilon());
				if (j > 0)
					_imp_matrix->imp_mat_data[i + (fft_size[1] - j) * fft_size[0] + k * fft_size[0] * fft_size[1]] = _g_temp[0];

				for (int l = 0; l < 3; l++) _r_temp[l] = r[l];
				_r_temp[2] = -_r_temp[2];
				FieldStaticScalar::get_G(_g_temp, _r_temp, Field::epsilon());
				if (k > 0)
					_imp_matrix->imp_mat_data[i + j * fft_size[0] + (fft_size[2] - k) * fft_size[0] * fft_size[1]] =  _g_temp[0];

				for (int l = 0; l < 3; l++) _r_temp[l] = -r[l];
				_r_temp[2] = -_r_temp[2];
				FieldStaticScalar::get_G(_g_temp, _r_temp, Field::epsilon());
				if ((i > 0) && (j > 0))
					_imp_matrix->imp_mat_data[(fft_size[0] - i) + (fft_size[1] - j) * fft_size[0] + k * fft_size[0] * fft_size[1]] =  _g_temp[0];

				for (int l = 0; l < 3; l++) _r_temp[l] = -r[l];
				_r_temp[0] = -_r_temp[0];
				FieldStaticScalar::get_G(_g_temp, _r_temp, Field::epsilon());
				if ((j > 0) && (k > 0))
					_imp_matrix->imp_mat_data[i + (fft_size[1] - j) * fft_size[0] + (fft_size[2] - k) * fft_size[0] * fft_size[1]] =  _g_temp[0];

				for (int l = 0; l < 3; l++) _r_temp[l] = -r[l];
				_r_temp[1] = -_r_temp[1];
				FieldStaticScalar::get_G(_g_temp, _r_temp, Field::epsilon());
				if ((k > 0) && (i > 0))
					_imp_matrix->imp_mat_data[(fft_size[0] - i) + j * fft_size[0] + (fft_size[2] - k) * fft_size[0] * fft_size[1]] =  _g_temp[0];

				for (int l = 0; l < 3; l++) _r_temp[l] = -r[l];
				FieldStaticScalar::get_G(_g_temp, _r_temp, Field::epsilon());
				if ((i > 0) && (j > 0) && (k > 0))
					_imp_matrix->imp_mat_data[(fft_size[0] - i) + (fft_size[1] - j) * fft_size[0] + (fft_size[2] - k) * fft_size[0] * fft_size[1]] =  _g_temp[0];

			}
	
	fftw_complex *_fft_in_f = NULL, *_fft_out_f = NULL;
	...
	for (int i = 0; i < _num_padded_grid_pts; i++)
	{
		_fft_in_f[i][0] = _imp_matrix->imp_mat_data[i];
		_fft_in_f[i][1] = 0.0f;
	}
	...
	for (int i = 0; i < _num_padded_grid_pts; i++)
	{
		_imp_matrix->k_imp_mat_data[i] = std::complex<FP_TYPE>(_fft_in_f[i][0], _fft_in_f[i][1]);
	}
	...
	for (int k = 0; k < _imp_matrix->_padded_green_dim[2]; k++)
		for (int j = 0; j < _imp_matrix->_padded_green_dim[1]; j++)
			for (int i = 0; i < _imp_matrix->_padded_green_dim[0]; i++){
				...
				_imp_matrix->k_imp_mat_data_gpu[green_id] = _fft_in_f[fft_id][0];
			}
	//BEN ADDED ABOVE
...
int NufftStaticScalar :: projection()
{
...
#pragma omp parallel for
	for (int i = 0; i < total_num_boxes; i++)
	{
		for (int m = src_box_map[i]; m < src_box_map[total_num_boxes + i]; m++)
		{
			...
			{
				u_src_grid[src_interp_idx[j + m * interp_nodes_per_box]] += src_interp_coeff[j + m * interp_nodes_per_box] * field_static_scalar->src_amp[m];
			} // loop j
		} // loop m
	} // loop i
...
int NufftStaticScalar :: fft_real_to_k()
{
#pragma omp parallel for
	for (int kk = 0; kk < fft_size[2]; kk++)
		for (int jj = 0; jj < fft_size[1]; jj++)
			for (int ii = 0; ii < fft_size[0]; ii++)
			{
				...
				fft_in_f[_idx1][0] = 0.0f;
				fft_in_f[_idx1][1] = 0.0f;

				if (ii < num_grid_pts[0] && jj < num_grid_pts[1] && kk < num_grid_pts[2])
				{
					fft_in_f[_idx1][0] = u_src_grid[_idx2];
				}
			}

	fftw_execute(plan_grid[0]);

	return 0;
}
int NufftStaticScalar :: convolution_k()
{
	...
	for (int kk = 0; kk < fft_size[2]; kk++)
		for (int jj = 0; jj < fft_size[1]; jj++)
			for (int ii = 0; ii < fft_size[0]; ii++)
			{
				...
				fft_in_b[_idx1][0] = fft_out_f[_idx1][0] * _impedance_matrix[_idx1].real() - fft_out_f[_idx1][1] * _impedance_matrix[_idx1].imag();

				fft_in_b[_idx1][1] = fft_out_f[_idx1][0] * _impedance_matrix[_idx1].imag() + fft_out_f[_idx1][1] * _impedance_matrix[_idx1].real();

			}
	return 0;
}
int NufftStaticScalar :: fft_k_to_real()
{
	fftw_execute(plan_grid[FIELD_DIM]);
#pragma omp parallel for
	for (int kk = 0; kk < num_grid_pts[2]; kk++)
		for (int jj = 0; jj < num_grid_pts[1]; jj++)
			for (int ii = 0; ii < num_grid_pts[0]; ii++)
			{
				...
				u_obs_grid[_idx2] = fft_out_b[_idx1][0] / FP_TYPE(total_num_fft_pts);
			}
	return 0;
}
int NufftStaticScalar :: correction_interpolation()//!!!!!!!!!!!!!NEAR CHANGE!!!!!!!!!!!!!WORK DONE
{
	int num_near_box_per_dim = 2*near_correct_layer+1;
	int total_num_near_box_per_box = num_near_box_per_dim*num_near_box_per_dim*num_near_box_per_dim;
...
//#pragma omp parallel for
//	for (int i = 0; i < interp_nodes_per_box; i++)
//	{
//		u_near_obs_grid[i] = 0.0f;
//	}
//
//#pragma omp parallel for
//	for (int i = 0; i < total_num_near_grid_pts; i++)
//	{
//		k_u_near_src_grid[i] = std::complex<FP_TYPE>(0.0f, 0.0f);
//	}
#pragma omp parallel for num_threads( omp_get_num_procs() )
	for (int i = 0; i < total_num_boxes; i++)
	{

		for (int j = 0; j < total_num_near_grid_pts; j++)
		{
		...
		for (int j = 0; j < near_box_list[i * (total_num_near_box_per_box+1)]; j++)
		{
			...
			for (int m = src_box_map[_current_near_box_idx]; m < src_box_map[total_num_boxes + _current_near_box_idx]; m++, _current_near_idx++)
			{
				for (int n = 0; n < interp_nodes_per_box; n++)
				{
					...
					for (int k = 0; k < 3; k++)
					{
						_node_idx_dim[k] -= (_box_idx_dim[k] - near_correct_layer) * (num_nodes[k] - 1);//SIDI TESTING (_box_idx_dim[k] - 1) * (num_nodes[k] - 1);
					}
					_node_idx = _node_idx_dim[0] + _node_idx_dim[1] * near_num_grid_pts[0] + _node_idx_dim[2] * near_num_grid_pts[0] * near_num_grid_pts[1];
					u_near_src_grid[_node_idx] += src_interp_coeff[n + m * interp_nodes_per_box] * field_static_scalar->src_amp[m];
		...
		for (int kk = 0; kk < 2 * near_num_grid_pts[2]; kk++)
			for (int jj = 0; jj < 2 * near_num_grid_pts[1]; jj++)
				for (int ii = 0; ii < 2 * near_num_grid_pts[0]; ii++)
				{
					...
					fft_in_f_near[_idx1][0] = 0.0f;
					fft_in_f_near[_idx1][1] = 0.0f;
					
					if (ii < near_num_grid_pts[0] && jj < near_num_grid_pts[1] && kk < near_num_grid_pts[2])
					{
						fft_in_f_near[_idx1][0] = u_near_src_grid[_idx2];
					}
				}
//forward fft
		fftw_execute(plan_grid_near[0]);
		...
		for (int kk = 0; kk < 2 * near_num_grid_pts[2]; kk++)
			for (int jj = 0; jj < 2 * near_num_grid_pts[1]; jj++)
				for (int ii = 0; ii < 2 * near_num_grid_pts[0]; ii++)
				{
					...
					fft_in_b_near[_idx1][0] = fft_out_f_near[_idx1][0] * _impedance_matrix_near[_idx1].real() - fft_out_f_near[_idx1][1] * _impedance_matrix_near[_idx1].imag();
					fft_in_b_near[_idx1][1] = fft_out_f_near[_idx1][0] * _impedance_matrix_near[_idx1].imag() + fft_out_f_near[_idx1][1] * _impedance_matrix_near[_idx1].real();
				}
//backward fft
		fftw_execute(plan_grid_near[FIELD_DIM]);
//post-fft
		for (int kk = 0; kk < num_nodes[2]; kk++)
			for (int jj = 0; jj < num_nodes[1]; jj++)
				for (int ii = 0; ii < num_nodes[0]; ii++)
				{
					...
					u_near_obs_grid[_idx1] = fft_out_b_near[_idx2][0] / 8.0f / total_num_near_grid_pts;
				}
//subtraction and interpolation
		for (int m = obs_box_map[i]; m < obs_box_map[total_num_boxes + i]; m++)
		{
			for (int j = 0; j < interp_nodes_per_box; j++)
			{
				...
				field_static_scalar->field_amp[m] += (u_obs_grid[_idx1] - u_near_obs_grid[_idx2]) * obs_interp_coeff[j + m * interp_nodes_per_box];
			}
		}
		...
		for (int l = obs_box_map[i]; l < obs_box_map[total_num_boxes + i]; l++)
		{
			...
			for (int j = 0; j < near_box_list[i * (total_num_near_box_per_box+1)]; j++)
			{
				...
				for (int m = src_box_map[_current_near_box_idx]; m < src_box_map[total_num_boxes + _current_near_box_idx]; m++, _current_near_idx++)
				{
					...
					{
						field_static_scalar->field_amp[l] += field_static_scalar->src_amp[m] * g_direct_near[l *max_near_src_cnt + _current_near_idx];
					}	
					...
					{	
						...
						for (int k = 0; k < 3; k++)
						{
							_temp_R[k] = field_static_scalar->obs_coord[l + problem_size * k] - field_static_scalar->src_coord[m + problem_size * k];
						}

						FieldStaticScalar::get_G(_temp_G, _temp_R, Field::epsilon());

						field_static_scalar->field_amp[l] += field_static_scalar->src_amp[m] * _temp_G[0];
