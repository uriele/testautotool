...
FieldStaticScalar :: FieldStaticScalar(class NBODYFAST *n_ptr) : Field(n_ptr)
{
	src_amp = NULL;
	field_amp = NULL;
	src_amp_dev = NULL;
...
FieldStaticScalar :: ~FieldStaticScalar()
{
...
int FieldStaticScalar :: array_alloc_multi_interface()
{
	nbodyfast->error->last_error = amp_field_alloc_multi();
	return 0;
}
int FieldStaticScalar :: amp_field_alloc()
{
	nbodyfast->memory->alloc_host<FP_TYPE>(&src_amp, nbodyfast->problem_size, "field->src_amp");
	nbodyfast->memory->alloc_host<FP_TYPE>(&field_amp, nbodyfast->problem_size, "field->field_amp");


	return 0;
}

int FieldStaticScalar :: set_src_amp(double *_charge)
{
...
#pragma omp parallel for
		...
		{
			src_amp[i] = FP_TYPE(_charge[src_trans_idx[i]]);
			field_amp[i] = 0.0f;
		}
		return 0;
	}
...
int FieldStaticScalar :: set_fld_amp(double *_field)
{
	// source copying for nufft method
	// sources are reordered according to src_trans_idx matrix
	if (nbodyfast->algo_name == "nufft")
...
int FieldStaticScalar :: amp_field_alloc_multi()
{
	...
	for (int _thread_id = 0; _thread_id < nbodyfast->num_devices; _thread_id ++)
	{
		...
		{
			nbodyfast->memory->alloc_host<FP_TYPE>(&src_amp_dev[_thread_id], nbodyfast->src_size_dev[_thread_id], _array_name.str());
		}
		...
		{
			nbodyfast->memory->alloc_host<FP_TYPE>(&field_amp_dev[_thread_id], nbodyfast->obs_size_dev[_thread_id], _array_name.str());
		}
	}
	return 0;
}
